    ...
    while screen_y < height {
        ...
        while screen_x < width {
            ...
            draw.draw_rectangle(
                ...
                block,
                block,
                floor_color(map, world_x, world_y),
            );
...
pub fn draw_walls(draw: &mut RaylibDrawHandle, width: i32, height: i32, rays: &[RayHit], fov: f32) {
    if rays.is_empty() {
        return;
    }
    ...
    for (index, ray) in rays.iter().enumerate() {
        ...
        let base_color = wall_color(ray.tile);
...
fn floor_color(map: &[Vec<char>], x: f32, y: f32) -> Color {
    if x < 0.0 || y < 0.0 {
        return grass_color();
    }
